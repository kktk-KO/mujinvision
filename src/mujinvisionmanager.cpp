...
std::string _GetExtraCaptureOptions(const std::vector<std::string>& cameraids, const std::vector<std::string>& cameraidstocheckocclusion, const ptree& visionserverpt, const std::string& controllerip, int binpickingTaskZmqPort, const std::string& slaverequestid, const std::map<std::string, std::string>& mCameraNameHardwareId, const std::map<std::string, std::string>& mCameranameActiveRegionname, const std::string& subscriberid, const bool ignoreocclusion=false, const std::string& bindetectionMode="never")
{
    ...
    extraoptionspt.put<std::string>("bindetectionmode", bindetectionMode);
...
void MujinVisionManager::_ExecuteUserCommand(const ptree& command_pt, std::stringstream& result_ss)
{
    ...
    if (command == "StartDetectionLoop" ||
        ...
        if (command == "StartDetectionLoop") {
            ...
            unsigned int numthreads = command_pt.get<uint32_t>("numthreads", 0);
            std::string cycleindex = command_pt.get<std::string>("cycleindex", "0");
        ...
        if (command == "Initialize") {
            ...
            std::string bindetectionMode = command_pt.get<std::string>("bindetectionmode", "never");
...
void MujinVisionManager::_DetectionThread(const std::string& regionname, const std::vector<std::string>& cameranames, DetectionThreadParams params)
{
    ...
    time0 = GetMilliTime();
    pBinpickingTask->Initialize(_defaultTaskParameters, _binpickingTaskZmqPort, _binpickingTaskHeartbeatPort, _zmqcontext, false, 0, _controllerCommandTimeout, userinfo_json, _slaverequestid);
    ...
    try {
        ...
        while (!_bStopExecutionVerificationPointCloudThread && evcamnames.size() > 0) {
            ...
            for (unsigned int i=0; i<evcamnames.size(); ++i) {
                ...
                int isoccluded = -1;
                isoccluded = _pImagesubscriberManager->GetCollisionPointCloud(cameraname, points, cloudstarttime, cloudendtime, newpointsize, filteringstddev, filteringnumnn, regionname, timeout, 0, filteringsubsample, 4);
                if (isoccluded == -2 ) {
    ...
    )
{
    ...
    if (_visionserverpt.count("bindetectionMode") > 0) {
        bindetectionMode = _visionserverpt.get<std::string>("bindetectionMode", "once");
        ...
        bindetectionMode = _visionserverpt.get<std::string>("bindetection", "once");
    }
    ...
    std::string detectormodulename = detectorconfigpt.get("detection.modulename", "");
    std::string detectorclassname = detectorconfigpt.get("detection.classname", "");
    ...
    _pBinpickingTask->Initialize(defaultTaskParameters, binpickingTaskZmqPort, binpickingTaskHeartbeatPort, _zmqcontext, false, 0, _controllerCommandTimeout, _userinfo_json, slaverequestid);
...
int MujinVisionManager::_DetectObjects(ThreadType tt, BinPickingTaskResourcePtr pBinpickingTask, const std::string& regionname, const std::vector<std::string>&cameranames, std::vector<DetectedObjectPtr>& detectedobjects, std::string& resultstate, unsigned long long& imageStartTimestamp, unsigned long long& imageEndTimestamp, int& isContainerPresent, const bool ignoreocclusion, const unsigned long long newerthantimestamp, const unsigned int fetchimagetimeout, const bool fastdetection, const bool bindetection, const bool request, const bool useold, const bool checkcontaineremptyonly, const std::string& cycleindex)
{
    ...
    bool bGotAllImages = _GetImages(tt, pBinpickingTask, regionname, colorcameranames, depthcameranames, colorimages, depthimages, resultimages, imageStartTimestamp, imageEndTimestamp, ignoreocclusion, newerthantimestamp, fetchimagetimeout, request, useold, waitinterval, 1);
...
void MujinVisionManager::_SendPointCloudObstacleToController(const std::string& regionname, const std::vector<std::string>&cameranames, const std::vector<DetectedObjectPtr>& detectedobjectsworld, const unsigned long long newerthantimestamp, const unsigned int fetchimagetimeout, const std::string& obstaclename, const bool fast, const bool request, const bool async, const std::string& locale)
{
    ...
    try {
        ...
        if (!async) {
            ...
            unsigned long long imageStartTimestamp = 0, imageEndTimestamp = 0;
            bool bGotAllImages = _GetImages(TT_Command, _pBinpickingTask, regionname, dummycameranames, depthcameranames, dummyimages, depthimages, dummyimages, imageStartTimestamp, imageEndTimestamp, ignoreocclusion, newerthantimestamp, fetchimagetimeout, request, false, 50, 2);
            if (bGotAllImages && depthimages.size() == depthcameranames.size()) {
                ...
                for (size_t i=0; i<depthimages.size(); ++i) {
                    ...
                    isoccluded = _pImagesubscriberManager->GetCollisionPointCloud(cameraname, points, cloudstarttime, cloudendtime, pointsize, filteringstddev, filteringnumnn, regionname, timeout, newerthantimestamp, filteringsubsample, 2);
                    if (points.size() / 3 == 0) {
                        ...
                        while (numretries > 0 && points.size() / 3 == 0) {
                            ...
                            bool bGotAllImages2 = _GetImages(TT_Command, _pBinpickingTask, regionname, dummycameranames, depthcameranames1, dummyimages, depthimages1, dummyimages, imageStartTimestamp, imageEndTimestamp, ignoreocclusion, newerthantimestamp, fetchimagetimeout, request, false, 50, 2);
                            if( bGotAllImages2 ) {
                                isoccluded = _pImagesubscriberManager->GetCollisionPointCloud(cameraname, points, cloudstarttime, cloudendtime, pointsize, filteringstddev, filteringnumnn, regionname, timeout, newerthantimestamp, filteringsubsample, 2);
                            }
                            numretries--;
...
void MujinVisionManager::_SendPointCloudObstacleToControllerThread(SendPointCloudObstacleToControllerThreadParams params)
{
    ...
    try {
        ...
        while (!_bStopSendPointCloudObstacleToControllerThread) {
            ...
            bool bGotAllImages = _GetImages(TT_SendPointcloudObstacle, pBinpickingTask, regionname, dummycameranames, depthcameranames, dummyimages, depthimages, dummyimages, imageStartTimestamp, imageEndTimestamp, ignoreocclusion, newerthantimestamp, fetchimagetimeout, true, false, 50, 2); // TODO update GetCollisionPointCloud() to remove this
            ...
            if (bGotAllImages && depthimages.size() == depthcameranames.size()) {
                ...
                for (size_t i=0; i<depthimages.size(); ++i) {
                    ...
                    points.resize(0);
                    int isoccluded = _pImagesubscriberManager->GetCollisionPointCloud(cameraname, points, cloudstarttime, cloudendtime, pointsize, filteringstddev, filteringnumnn, regionname, timeout, 0, filteringsubsample, 2);
                    ...
                    if (points.size() / 3 == 0) {
                        ...
                        while (numretries > 0 && points.size() / 3 == 0) {
                            ...
                            if( bGotAllImages2 ) {
                                isoccluded = _pImagesubscriberManager->GetCollisionPointCloud(cameraname, points, cloudstarttime, cloudendtime, pointsize, filteringstddev, filteringnumnn, regionname, timeout, 0, filteringsubsample, 2);
                            }
                            numretries--;
                        }
                        if (points.size() / 3 == 0) {
...
void MujinVisionManager::_VisualizePointCloudOnController(const std::string& regionname, const std::vector<std::string>&cameranames, const double pointsize, const bool ignoreocclusion, const unsigned long long newerthantimestamp, const unsigned int fetchimagetimeout, const bool request)
{
    ...
    for (unsigned int i=0; i<cameranamestobeused.size(); i++) {
        ...
        unsigned long long imageStartTimestamp = 0, imageEndTimestamp = 0;
        bool bGotAllImages = _GetImages(TT_Command, _pBinpickingTask, "", dummycameranames, dcamnames, dummyimages, depthimages, dummyimages, imageStartTimestamp, imageEndTimestamp, ignoreocclusion, newerthantimestamp, fetchimagetimeout, request, false, 50, 8);
        ...
        _pImagesubscriberManager->GetCollisionPointCloud(cameraname, points, cloudstarttime, cloudendtime, _filteringvoxelsize, _filteringstddev, _filteringnumnn, "", 20.0, 0, 1, 8);
        if (points.size()>0) {
