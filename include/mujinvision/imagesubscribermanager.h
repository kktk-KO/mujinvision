...
public:
    ...
    /** \brief Initializes the image subscriber manager.
        ...
        \param params_pt boost property tree defining the image subscriber parameters
     */
    ...
    /** \brief stops capture thread
        \param checkpreemptbits specifying which bit to check for preempting
     */
    virtual void StopCaptureThread(const std::vector<std::string>& cameranames, const double timeout=5.0, const unsigned int checkpreemptbits=0) = 0;
